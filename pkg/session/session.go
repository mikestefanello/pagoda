...
func Get(ctx echo.Context, name string) (*sessions.Session, error) {
	...
	// Errors are ignored because it's possible the cookie value was invalid or the encryption key
	// changed since the cookie was written. A usable session is always returned even if a new one
	// has to be created.
